...
class FilterController extends Default_Backend_Controler
{
    ...
    public function update($id)
    {
        ...
        if (!$this->update_all_slug($id, $SELECT_FILTER_DATA, $tagSlug,$type)) {
            $this->db->trans_rollback();
            exit(json_encode([
                'responseCode' => 503,
                'responseStatus' => false,
    ...
    public function update_all_slug($ID, $SELECT_FILTER_DATA, $NEW_SLUG,$Type)
    {
        $id = $ID;
        $URL_SLUG = $SELECT_FILTER_DATA['URL_SLUG'];
        $UPDATE_SLUG = $NEW_SLUG;
        $allChild = $this->db->select('*')->like('URL_SLUG', $URL_SLUG)->where('DB_ID !=', $id)->where('TYPE', $Type)->get('filter');
